...
const Footer = () => {
  return (
    <section className='w-full bg-[var(--bgCL)] relative overflow-hidden sm:px-8 md:px-12 lg:px-16  border-t border-[var(--border)]'>
      <div className='flex flex-col sm:flex-row sm:justify-between sm:items-start gap-8 sm:gap-10 py-8 sm:py-10'>
        ...
        <div className='w-full sm:max-w-100'>
          ...
          <p className='font-medium text-lg lg:text-xl text-[var(--mainText)] mt-2'>
            Creating{' '}
              ...
              {navigationLinks.map((link, id) => {
                return (
                  <a
                    href={link.href}
                    key={id}
                    className='block text-[var(--secoundaryText)] font-medium hover:text-[var(--mainText)] '
                  >
                    {link.text}
                  </a>
                );
